...
void Initialize( Maze **maze, int nowline, int nowcolumn, int column, int line ) {
	while ( nowline < line ) {
		while ( nowcolumn < column ) {
			...
			maze[nowline][nowcolumn].foundg = false ;
			nowcolumn++ ;
		} // while
		nowline++ ;
		nowcolumn = 0 ;
...
void PutWrong( Maze **maze, int &nowline, int &nowcolumn, int column, int line ) {
	...
	while ( maze[nowline][nowcolumn].otheroads == 0 ) {
		if ( nowline == 0 && nowline == 0 )
		  break ;
		if ( maze[nowline][nowcolumn].step == 'G' || maze[nowline][nowcolumn].foundg )
			findg = true ;
...
bool Walk( int goal, Maze **maze, int nowline, int nowcolumn, int column, int line ) {
    int count = 0 ;
    Maze *p[10] ;
	while ( nowline < line ) {
...
int main() {
	...
	success = Walk( goal, p, nowline, nowcolumn, column, line ) ;
