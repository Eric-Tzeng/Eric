...
void Postfix( FormulaPtr formula, FormulaPtr count, bool first, FormulaPtr formula2  ) {
	FormulaPtr temp ;
	FormulaPtr temp2 ;
	if  ( formula != NULL ) {
		if ( formula->word == 'n' ) {
			...
			formula2->word = formula->word ;
			formula2->num = formula->num ;
			formula2->next = new Formula() ;
			Postfix( formula->next, count, first, formula2->next ) ;
		} // if
		else {
			if ( formula->word == ')' ) {
				temp2 = formula2 ;
				while ( count->word != '(' ) {
					cout << ", " << count->word ;
					temp2->word = count->word ;
					temp2->next = new Formula() ;
					temp2 = temp2->next ;
					count = count->next ;
				} // while
				
				count = count->next ;
				Postfix( formula->next, count, first, temp2 ) ;
			} // if
				...
				count->next = temp ;
				Postfix( formula->next, count, first, formula2 ) ;
			} // if
			else if ( formula->word == '+' || formula->word == '-' ) {
				if( count->word == '+' || count->word == '-' ) {
					cout << ", " << count->word ;
					formula2->word = count->word ;
					formula2->next = new Formula() ;
					count->word = formula->word ;
					Postfix( formula->next, count, first, formula2->next ) ;
				} // if
					...
					Postfix( formula, count, first, temp ) ;
				} // else
			} // if
...
void Answer( FormulaPtr formula, FormulaPtr number ) {
	FormulaPtr temp ;
	int num = 0 ;
	if ( formula->word == 'n' ) {
		number->num = formula->num ;
		number->next = new Formula() ;
		Answer( formula->next, number->next ) ;
	} // if
	else if ( Operator( formula ) ) {
...
int main() {
	...
	if ( Check( formula, openparenthesis, closeparenthesis ) ) {
        ...
        Postfix( formula2, count, first, formula ) ;
        delete formula2 ;
        delete count ;
        cout << '\n' ;
        Print( formula ) ;
        count = NULL ;
        Answer( formula, count ) ;
	} // if 
